    ...
    >
      <div className="md:container relative md:top-20 z-20 flex flex-col md:max-w-screen-xl">
        <div className="container grid md:grid-cols-2 items-center justify-between gap-4 md:gap-2">
          <h1
            id="about-title"
            className="lg:text-4xl text-center md:text-left text-2xl font-head text-theme-blue"
          >
            About Short Form Books
          </h1>
          <p
            id="about-description"
            className="text-[12px] text-center md:text-left lg:text-base font-head text-black"
          >
...
const DesktopCard = ({
  ...
  return (
    <div className="desktop-card h-[368px] relative snap-center px-3 md:px-0">
      <div
        id={`desktop-card-${i}`} // Added ID here
        onMouseEnter={() => setSelectedCard(i)}
        className={`card-content flex h-[320px] w-full  rounded-lg shadow-lg bg-white border-t-4 flex-col gap-3 p-4 transition-all duration-300
      ${
      ...
      >
        ...
        <h1
          ...
          className={`card-title text-base lg:text-2xl font-head text-center ${
            i === selected ? "text-theme-blue" : "text-black"
          }`}
        >
          {card.title}
        ...
        <p
          ...
          className="card-description text-[12px] lg:text-sm font-body text-center text-black lg:mt-6"
        >
          {card.description}
        </p>
      </div>
...
const MobileCard = ({ card, i }: { card: WhyCard; i: number }) => {
  ...
  return (
    <div
      ...
      className={`mobile-card snap-centers relative  md:hidden bottom-0 h-[300px] w-[300px] md:w-full rounded-lg shadow-lg bg-white border-t-4 flex flex-col gap-3 p-4 transition-all duration-300
    ${
      displayed
